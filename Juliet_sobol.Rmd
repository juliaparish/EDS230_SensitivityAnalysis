...
k = rnorm(mean = k0_value, sd = k0_sd, n = nsets)
kd = rnorm(mean = kd_value, sd = kd_sd, n = nsets)
vm = rnorm(mean = vm_mean, sd = vm_sd, n = nsets)
h = runif(min = h_min, max = h_max, n = nsets)

X1 = cbind.data.frame(k, kd, vm, h)
...
sens_atmcon_Sobol = sobolSalt(model = NULL, X1, X2, nboot = 100)
...
# convert the list putput into a df using mapdf as in her code in part b
# as.matrix
model_output = pmap(parms, atmcon)
# turn this list into a vector ???
#model_output_vector <-()

sens_Catm_Sobol = sensitivity::tell(sens_atmcon_Sobol, model_output, res.names = "ga")
